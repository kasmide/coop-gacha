...
import coop_gacha.model.{Menu, MenusInfo}
  ...
  renderOnDomContentLoaded(
    dom.document.getElementById("app"), {
      ...
      dom
        ...
        .map { menusInfo =>
          val menuDetails =
            menusInfo.asInstanceOf[MenusInfo].menu_details.toArray
          ...
          dom.window.location.hash match {
            case url if url != "" =>
              val menuIDs =
                js.URIUtils.decodeURIComponent(url.drop(1)).split(",")
              val menus = menuIDs
                .map(menu => {
      ...
      div(
        kondateConfig.signal.changes --> { _ => kondateSolver.set(None) },
        solvedKondate.signal.changes --> {
          case Some(menus) =>
            val menusString =
              menus
                .map(menu => s"${menu.houseId}:${menu.menuId}")
                .mkString(",")
          ...
          label(
            input(
              ...
              onChange.mapToChecked --> { checked =>
                kondateConfig.update(conf =>
                  conf.copy(requireMainDish = checked)
                )
              }
            ),
            "主菜(200円以上)を必ず含める"
